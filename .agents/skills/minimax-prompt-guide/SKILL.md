...
1. **Idioma de Falas/Diálogos, Sintaxe Oficial `<d>` vs. Texto Gráfico `""` e Limite Temporal:**
   - Toda fala/diálogo na cena DEVE ser estritamente em **Português do Brasil**.
   - **Sintaxe Oficial de Falas vs. Texto na Tela (Substituição de Padrão):**
     * **Falas e Diálogos de Personagens:** DEVEM utilizar estritamente a tag oficial **`<d>[Portuguese] ...</d>`** com o identificador de speaker `(S1)`, `(S2)` (ex: `<Subject 1> (S1) speaks clearly in Brazilian Portuguese: <d>[Portuguese] Olá, sejam muito bem-vindos!</d>`). NUNCA colocar falas soltas entre aspas sem a tag `<d>`, pois isso induz o modelo a projetar legendas/caracteres flutuantes na tela.
     * **Narração / Voz em Off (Voiceover):** Quando a voz for de narração ou pensamento de um personagem em tela, DEVE-SE usar rigorosamente a fórmula oficial da MiniMax:
       ...
       *(A cláusula `while his lips remain completely closed` é mandatória para impedir que o modelo anime a boca do ator em cena inadvertidamente).*
     * **Falas em Uníssono:** Quando múltiplos sujeitos vocalizam juntos: `<Subject 1, Subject 2> (S1,S2) shout together, <d>[Portuguese] ...</d>`.
     * **Transições e Cortes de Fala (`<scenetrans>` e `<cutoff>`):**
       - Se a mesma fala atravessar um corte entre planos: usar `<scenetrans>` com `continues seamlessly across the cut` ou `carries over from the previous shot`.
       - Se a fala for cortada pelo término natural do vídeo de 10s: usar a tag `<cutoff>`.
     * **Texto Gráfico / Visível em Cena (Placas, Neons, Camisas, Banners, Letreiros):** DEVE ficar estritamente entre **aspas duplas inglesas `""`** (ex: *The neon sign on the brick wall reads "CAFÉ"*). Conforme o guia oficial, texto visual sem aspas colapsa em símbolos ilegíveis (*gibberish*).
...
3. **Aspect Ratio & Resolução (Múltiplos de 32):** Todo vídeo DEVE ser formatado em **9:16 (Vertical)** (resolução de sampling otimizada em `608x1056`, com upscale 4K vertical RTX VSR `2160x3840`).
   - *Aviso Crítico de Resolução:* Todas as dimensões devem ser múltiplos de 32. **É expressamente PROIBIDO selecionar o preset `1.0 MP` no `ResolutionSelector`**, pois ele gera dimensões que ultrapassam a área máxima do H3-Base (`768x1344`), corrompendo a geração. O padrão `0.6 MP` (`608x1056`) é seguro e ideal.
...
- `<Audio 1>`... (Áudio conectado ao `LoadAudio`). Deve conter declaração explícita de retenção:
  - `fully_copy`: reutiliza o áudio 1:1 na íntegra como trilha final e garante o sincronismo labial exato (*lipsync*).
  - `reference`: extrai apenas o timbre vocal ou estilo musical para uma nova fala especificada na tag `<d>`.
- **Cenas com Múltiplos Personagens (Prevenção Rigorosa de Mistura e Troca de Lados/Roupas):** 
  - *Ancoragem Espacial Inicial Left-to-Right:* Em enquadramentos com 2 ou mais pessoas em formato 9:16, SEMPRE declarar a posição física exata de cada sujeito da esquerda para a direita (*Left to Right*: ex: *far-left, center-left, center-right, far-right*).
  - *Protocolo Anti-Drift Per-Beat:* Em cada bloco temporal (ex: 0s-4s, 4s-8s, 8s-10s), reancorar expressamente a cor/tipo do figurino e o lado de cada personagem (`still on camera-left in his dark suit`, `remaining on camera-right in her beige cardigan`), instruindo que giros ou movimentos ocorram em seus próprios eixos sem cruzar a linha de ação.
  - *Anti-Drift em Motion Transfer:* Reancorar o cenário da cena a cada bloco temporal para impedir que o fundo do vídeo de movimento sangre para a tomada.
...
2. **[2. Scene Breakdown & Action Timeline (0s - 10s)]:**
   > Descreva ações físicas e tangíveis divididas no tempo, com textos em cena entre aspas duplas `""` e falas rigorosamente em tags `<d>[Portuguese] ...</d>` (respeitando o teto de 15 a 20 palavras totais):
   > *"- (0s - 4s): <Subject 1> executes the starting athletic movements matching <Video 1> in a modern storefront with a sign reading 'ACADEMIA'.*
   > *- (4s - 8s): <Subject 1> (S1) turns to camera and speaks in Brazilian Portuguese: <d>[Portuguese] Olá pessoal, hoje vamos ver as novidades!</d> (8 palavras - cadência natural sem atropelo).*
   > *- (8s - 10s): <Subject 1> completes the movement sequence smoothly, holding eye contact as lighting subtly shifts."*

3. **[3. Camera Movement]:**
   > Explicite a evolução de enquadramento e movimento dinâmico ao longo dos 10 segundos, banindo o ângulo frontal estático, aplicando corpos/lentes de cinema reais (ARRI Alexa, Cooke S4, Zeiss Ultra Prime) e articulando as três dimensões oficiais da MiniMax: **Motion Type** (`Push In / Pull Out`, `Pan Left / Right`, `Truck Left / Right`, `Tilt Up / Down`, `Pedestal Up / Down`, `Arc Shot`, `Tracking Shot`, etc.) + **Amplitude** (`with small amplitude` / `with large amplitude`) + **Velocidade** (`at slow speed` / `at fast speed`):
   > *"(0s-3s): Shot on an ARRI Alexa with a Cooke S4 35mm prime in a high-angle slightly elevated establishing shot, the camera pedestals down with small amplitude at slow speed, (3s-7s): dynamic over-the-shoulder (OTS) framing tracking backward parallel at moderate speed, (7s-10s): slow cinematic arc pushing in with small amplitude at slow speed toward the main character's facial expression with natural optical bokeh."*

4. **[4. Overall Soundscape (Diegetic Audio)]:**
   > Sons físicos do ambiente e objetos (1 a 4 frases contínuas em inglês):
   > *"Natural room tone, crisp voice acoustics, tactile fabric rustle when moving hands, soft footsteps on hardwood floor."*

5. **[5. Non-Diegetic Music (Atmosphere)]:**
   > Trilha musical estritamente focada em **instrumentação, andamento (tempo), ritmo e dinâmica física** (1 a 3 frases em inglês). **PROIBIDO usar adjetivos abstratos de emoção/humor** (como "inspiring", "dramatic emotion", "sad"). Caso não haja trilha: usar estritamente `N/A`:
   > *"Sparse acoustic piano notes at a slow tempo, accompanied by sustained low cello strings that gently swell before fading out."*

### E. Fórmulas de Alinhamento Obrigatório para Modos Base de Imagem (I2VA / FL2VA / L2VA)
Se a solicitação envolver animação a partir de imagem em vez do modo Full-Reference:
- **I2VA (1º Frame):** A primeiríssima linha do prompt DEVE ser:
  > `For the target video, at 0.00 seconds into the target video, <Picture 1> (from [Shot 1]) is fully referenced.`
- **FL2VA (1º e Último Frame com Interpolação):** A primeiríssima linha DEVE ser:
